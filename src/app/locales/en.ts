export const en = {
  common: {
    back: 'Back',
    cancel: 'Cancel',
    save: 'Save',
    saving: 'Saving...',
    loading: 'Loading...',
    confirm: 'Confirm',
    reject: 'Reject',
    edit: 'Edit',
    add: 'Add',
    delete: 'Delete',
    continue: 'Continue',
    yes: 'Yes',
    no: 'No',
    showPassword: 'Show password',
    hidePassword: 'Hide password',
    notFound: 'Entry not found.',
    occurrence: 'occurrence',
    occurrences: 'occurrences',
  },
  splash: {
    tagline: 'Your self-discovery journey',
  },
  login: {
    title: 'Welcome back',
    subtitle: 'Continue your self-discovery journey.',
    emailPlaceholder: 'Email',
    passwordPlaceholder: 'Password',
    forgotPassword: 'Forgot password',
    submit: 'Sign in',
    submitting: 'Signing in...',
    noAccount: "Don't have an account?",
    createAccount: 'Sign up',
    google: 'Continue with Google',
    apple: 'Continue with Apple',
    errors: {
      missingFields: 'Enter your email and password.',
      invalidCredentials: 'Incorrect email or password.',
      emailNotConfirmed: 'Confirm your email before signing in.',
      rateLimit: 'Too many attempts. Try again in a moment.',
      noAccount: 'No account found for this login. Please create an account first.',
    },
  },
  signup: {
    title: 'Create your account',
    subtitle: 'Start logging your emotions today.',
    namePlaceholder: 'Name',
    emailPlaceholder: 'Email',
    birthDatePlaceholder: 'Date of birth',
    nativeLanguage: 'Native language',
    nativeLanguageHint: 'Used for voice transcription and your insights. The interface stays in Portuguese or English.',
    passwordPlaceholder: 'Password (min. 6 characters)',
    submit: 'Create account',
    submitting: 'Creating...',
    hasAccount: 'Already have an account?',
    goToLogin: 'Sign in',
    google: 'Sign up with Google',
    apple: 'Sign up with Apple',
    acceptTerms:
      'I have read and agree to the <terms>Terms of Use</terms> and the <privacy>Privacy Policy</privacy>.',
    confirmAge: 'I confirm that I am 18 years of age or older.',
    successTitle: 'Account created!',
    successMessage: 'We sent a confirmation link to <strong>{{email}}</strong>. Check your inbox to continue.',
    goLogin: 'Go to sign in',
    errors: {
      missingFields: 'Fill in all fields.',
      shortPassword: 'Password must be at least 6 characters.',
      underage: 'You must be 18 or older to create an account.',
      mustConfirmAge: 'You must confirm that you are 18 or older.',
      alreadyRegistered: 'This email is already registered.',
      invalidEmail: 'Invalid email.',
      passwordInvalid: 'Invalid password — minimum 6 characters.',
      rateLimit: 'Too many attempts. Try again in a moment.',
    },
  },
  att: {
    title: 'Help Caminare grow',
    body: 'It helps us understand which paths bring people to Caminare and improve the app for those who need it. You decide on the next screen.',
    privacy: 'Your entries are never shared. Only anonymous app usage data.',
    continue: 'Continue',
    notNow: 'Not now',
  },
  forgotPassword: {
    title: 'Forgot your password?',
    subtitle: 'Enter your email and we will send a link to create a new password.',
    emailPlaceholder: 'Email',
    submit: 'Send recovery link',
    submitting: 'Sending...',
    remembered: 'Remember your password?',
    goBack: 'Back to sign in',
    sentTitle: 'Check your email',
    sentMessage: 'We sent a recovery link to <strong>{{email}}</strong>. Click the link to create a new password.',
    backToLogin: 'Back to sign in',
    errors: {
      missingEmail: 'Enter your email.',
      invalidEmail: 'Invalid email.',
      rateLimit: 'Too many attempts. Try again in a moment.',
    },
  },
  resetPassword: {
    title: 'Create new password',
    subtitle: 'Set a secure password for your account.',
    newPlaceholder: 'New password (min. 6 characters)',
    confirmPlaceholder: 'Confirm new password',
    submit: 'Save new password',
    successTitle: 'Password updated!',
    successMessage: 'Your password has been changed successfully. You can now sign in with the new password.',
    goToApp: 'Go to app',
    invalidTitle: 'Invalid or expired link',
    invalidMessage: 'The recovery link is invalid or has expired. Request a new one.',
    requestNew: 'Request new link',
    errors: {
      shortPassword: 'Password must be at least 6 characters.',
      mismatch: 'Passwords do not match.',
      sameAsOld: 'New password cannot be the same as the current one.',
      expired: 'Link expired or invalid. Request a new one.',
      passwordInvalid: 'Invalid password — minimum 6 characters.',
    },
  },
  home: {
    greeting: 'Hi, {{name}}',
    greetingFallback: 'there',
    prompt: 'What happened today?',
    stats: {
      emotions: 'Emotions',
      patterns: 'Patterns',
      days: 'Days',
      entries: 'Entries',
    },
    voiceButton: 'New Voice Entry',
    textButton: 'New Text Entry',
    patternModalTitle: '{{name}}, a possible new pattern has been recognized.',
    patternModalMessage: 'Want to analyze it now?',
    patternModalAnalyze: 'Analyze',
    patternModalLater: 'Later',
  },
  history: {
    title: 'History',
    subtitle: 'Your entries',
    searchPlaceholder: 'Search entries...',
    filterAll: 'All',
    filter7: 'Last 7 days',
    filter15: 'Last 15 days',
    filter30: 'Last 30 days',
    empty: 'No entries yet. Create your first one!',
    emptySearch: 'No entries found for your search.',
    edit: 'Edit entry',
    delete: 'Delete entry',
    deleteConfirm: 'Delete this entry? This action cannot be undone.',
    textLabel: 'Entry text',
    addEmotion: 'Add emotion',
    emotionPlaceholder: 'Type an emotion...',
    removeEmotion: 'Remove emotion',
  },
  entryDone: {
    title: 'Entry complete!',
    message: 'Your emotions and beliefs have been saved.',
    backHome: 'Back to home',
    backNow: 'Back now',
  },
  entryDetail: {
    title: 'Full entry',
    fullText: 'Entry text',
    emotionsTitle: 'Validated emotions',
    emotionsEmpty: 'No emotions validated yet.',
    thoughtsTitle: 'Identified thoughts',
    thoughtsEmpty: 'No thoughts identified yet.',
  },
  recording: {
    title: 'New Voice Entry',
    language: 'Speech language',
    transcriptionLabel: 'Real-time transcription',
    placeholderRecording: 'Start speaking...',
    placeholderIdle: 'Tap the button below to start recording your thoughts and emotions.',
    useTextInstead: 'Use text entry',
    startPrompt: 'Tap the microphone to start recording',
    recording: 'Recording',
    paused: 'Paused',
    pause: 'Pause',
    resume: 'Resume',
    stop: 'Stop',
    langPickerTitle: 'Transcription language',
    errorLangUnsupported: 'Language not supported in this browser. Try another.',
    save: 'Save',
    discard: 'Discard',
    rerecord: 'Record again',
    continueRecording: 'Continue recording',
    reviewSubtitle: 'Review your recording. You can save, record again, or discard.',
    saving: 'Saving entry...',
    limitNote: 'Maximum 2 minutes per recording.',
    errorPermission: 'Microphone permission denied. Enable it in your browser settings to record.',
    errorNetwork: 'Network error during speech recognition. Check your connection.',
    errorGeneric: 'Could not start recording. Try again.',
    errorEmpty: 'No speech detected. Try speaking closer to the microphone.',
    errorSave: 'Could not save the entry. Try again.',
    notSupported: 'Your browser does not support speech recognition yet. Use text entry instead.',
  },
  textRecording: {
    title: 'New Text Entry',
    placeholder: 'How are you feeling today? What happened?',
    voiceHint: 'To dictate, tap the field and use the microphone on your phone keyboard.',
    submit: 'Finish Entry',
    submitting: 'Saving...',
    errorSave: 'Could not save the entry. Try again.',
    errorGeneric: 'Error saving the entry.',
  },
  emotionValidation: {
    title: 'Validation',
    subtitle: 'Validate the emotions we identified.',
    subtitleManual: 'Validate the emotions we identified.',
    emotionsHeader: 'Identified emotions',
    addEmotion: 'Add emotion',
    newEmotionPlaceholder: 'Type an emotion...',
    emptyEmotions: 'No emotions yet. Add one below.',
    thoughtsHeader: 'Identified thoughts',
    continue: 'Continue',
    analyzing: 'Analyzing your entry...',
    analyzingSteps: [
      'Reading your entry carefully...',
      'Identifying possible emotions.',
      'Organizing the insights...',
      'Almost there...',
    ],
    analyzingBeliefs: 'Analyzing possible beliefs',
    analyzingBeliefsSteps: [
      'Reading your entry carefully...',
      'Analyzing possible beliefs',
      'Connecting the dots...',
      'Almost there...',
    ],
    analyzeError: 'We could not analyze your entry right now. You can still add emotions manually below.',
    missingEntry: 'Entry not found. Go back and try again.',
  },
  beliefValidation: {
    title: 'Identified Beliefs',
    subtitle: 'Validate possible beliefs that may be behind the emotional reaction to this event',
    subtitleManual: 'Validate possible beliefs that may be behind the emotional reaction to this event',
    adjust: 'Adjust',
    discard: 'Discard',
    hint: 'When images and beliefs operate unconsciously, disharmonies can arise and negative patterns can repeat.',
    finish: 'Finish Entry',
    addBelief: 'Add belief',
    newBeliefPlaceholder: 'e.g. I am not good enough...',
    empty: 'No beliefs recorded yet. Add one below.',
    analyzingPatterns: 'Analyzing patterns...',
  },
  newPattern: {
    title: 'New Pattern Identified',
    subtitle: 'We identified a possible new pattern. See if it makes sense.',
    badge: 'Pattern detected',
    descriptionLabel: 'Pattern description:',
    triggersLabel: 'Common triggers:',
    confirmPattern: 'Confirm Pattern',
    edit: 'Edit',
    notApplies: "Doesn't Apply",
    noneTitle: 'No new patterns',
    noneMessage: 'Keep recording your emotions and beliefs. When a recurring pattern emerges, you will see it here.',
    finish: 'Done',
  },
  patterns: {
    title: 'Insights',
    subtitle: 'Your emotions, beliefs and patterns',
    loading: 'Loading insights...',
    empty: 'Your insights appear here after you create and validate your first entries.',
    emotionsHeader: 'My top emotions',
    beliefsHeader: 'Recurring Beliefs',
    variation: 'variation',
    variations: 'variations',
    patternsHeader: 'Behavioral Patterns',
    patternBadge: 'Pattern detected',
    editBelief: 'Edit belief',
    deleteBelief: 'Delete belief',
    deleteBeliefConfirm: 'Delete this belief? This action cannot be undone.',
  },
  summary: {
    title: 'Therapy Summary',
    subtitle: "Don't forget to bring anything important to therapy.",
    period7: 'Last 7 days',
    period15: 'Last 15 days',
    period30: 'Last 30 days',
    headerCardTitle: 'Period Summary',
    statsHeader: 'General Statistics',
    totalEntries: 'Total entries in the period',
    topEmotions: 'Top emotions in the period',
    eventsHeader: 'Events since your last session',
    empty: 'No entries in this period.',
    downloadPdf: 'Download PDF',
    share: 'Share',
    pdfTitle: 'Therapy Summary',
    pdfStats: 'General statistics',
    pdfRecords: 'entries in the period',
    pdfEvents: 'Events since your last session',
    pdfFor: 'For',
    pdfPeriodLabel: 'Period',
    pdfGeneratedAt: 'Generated on',
    pdfPage: 'Page',
    pdfConfidential: 'Confidential · Personal use',
    pdfError: 'Could not generate the PDF. Try again.',
    generating: 'Generating...',
    shareText: 'My Caminare summary',
    pdfNativeDone: 'Done! Use the menu to save to Files or share.',
    pdfDialogTitle: 'Therapy Summary',
  },
  nameGate: {
    title: 'What should we call you?',
    subtitle: 'Enter your name to personalize your Caminare experience.',
    placeholder: 'Your name',
    error: 'Please enter a valid name.',
  },
  profile: {
    title: 'Profile',
    subtitle: 'Your information and settings',
    noName: 'No name',
    accountSection: 'Account',
    name: 'Name',
    email: 'Email',
    memberSince: 'Member since',
    loginMethod: 'Login method',
    emailPassword: 'Email and password',
    changePassword: 'Change password',
    sendFeedback: 'Send a suggestion',
    feedback: {
      placeholder: 'Write your suggestion...',
      submit: 'Send',
      submitting: 'Sending...',
      success: 'Suggestion sent. Thank you!',
      empty: 'Write your suggestion before sending.',
      errorGeneric: 'Could not send the suggestion. Try again.',
    },
    signOut: 'Sign out',
    deleteAccount: 'Delete account',
    legalSection: 'Documents',
    subscriptionSection: 'Subscription',
    manageSubscription: 'Manage subscription',
    cancelSubscription: 'Cancel subscription',
    editName: 'Edit name',
    namePlaceholder: 'Your name',
    editEmail: 'Edit email',
    emailPlaceholder: 'Your email',
    emailChangeSent:
      'We sent a confirmation link to the new email. The change will only take effect after you open that link. Check your current inbox as well.',
    accountTypeSection: 'Account type',
    accountTrial: 'Free trial',
    accountTrialDaysLeft_one: '{{count}} day left',
    accountTrialDaysLeft_other: '{{count}} days left',
    accountTrialEndsToday: 'Ends today',
    accountTrialDone: 'Trial ended',
    accountActive: 'Active subscription',
    accountTierBasico: 'Basic plan',
    accountTierAvancado: 'Advanced plan',
    accountCadenceMonthly: 'Monthly',
    accountCadenceAnnual: 'Annual',
    accountRenewsOn: 'Renews on {{date}}',
    accountRestricted: 'No active access',
    accountUsage: '{{used}} of {{limit}} entries this period',
    accountViewPlans: 'See plans',
    settingsSection: 'Preferences',
    language: 'Language',
    nativeLanguage: 'Native language',
    nativeLangCooldown: 'You can change your native language again in {{days}} days.',
    nativeLangTranslating: 'Language changed. Translating your insights to the new language...',
    nativeLangDone: 'Done. Your insights are now in the new language.',
    theme: 'Theme',
    themeLight: 'Light',
    themeDark: 'Dark',
    themeSystem: 'System',
    languagePT: 'Português',
    languageEN: 'English',
    currentPwdPlaceholder: 'Current password',
    newPwdPlaceholder: 'New password (min. 6 characters)',
    confirmPwdPlaceholder: 'Confirm new password',
    pwdSuccess: 'Password changed successfully!',
    errors: {
      missingFields: 'Fill in all fields.',
      mismatch: 'Confirmation does not match the new password.',
      sameAsOld: 'New password must be different from the current one.',
      shortPassword: 'New password must be at least 6 characters.',
      wrongCurrent: 'Current password is incorrect.',
      invalidSession: 'Invalid session. Sign in again.',
      emailInvalid: 'Enter a valid email.',
      emailSame: 'The new email must be different from the current one.',
      emailUpdateFailed: 'Could not change the email. Please try again.',
    },
    deleteModal: {
      title: 'Delete your account?',
      message: 'This action is permanent. All your entries, emotions, beliefs and patterns will be erased. This cannot be undone.',
      confirmLabel: 'Type <strong>DELETE</strong> to confirm',
      confirmWord: 'DELETE',
      submit: 'Delete account',
      submitting: 'Deleting',
      errorGeneric: 'Error deleting the account.',
    },
  },
  legal: {
    termsLink: 'Terms of Use',
    privacyLink: 'Privacy Policy',
  },
  admin: {
    title: 'Admin Panel',
    adminBadge: 'Admin mode',
    profileSubtitle: 'Stats, users and suggestions',
    backToApp: 'Back to app',
    refresh: 'Refresh',
    errorLoad: 'Could not load admin data.',
    nav: {
      overview: 'Overview',
      emotions: 'Emotions',
      beliefs: 'Beliefs',
      patterns: 'Patterns',
      feedback: 'Suggestions',
      users: 'Users',
    },
    period: {
      d7: '7 days',
      d30: '30 days',
      d90: '90 days',
      all: 'All',
    },
    overview: {
      kpiUsers: 'Users',
      kpiEntries: 'Entries',
      kpiAvgWeekly: 'Weekly average',
      kpiValidation: 'AI validation',
      qualityTitle: 'AI quality',
      viewDetails: 'View details',
      subscriptionsSoon: 'Subscriptions & stores — coming soon',
      subscriptionsBody: 'Available once the subscription module and store integration are enabled.',
      subscriptionsTitle: 'Subscriptions & stores',
      subsTrials: 'On trial',
      subsActive: 'Active subscribers',
      subsMonthly: 'Monthly plan',
      subsAnnual: 'Annual plan',
      subsApple: 'Apple accounts',
      subsGoogle: 'Google accounts',
    },
    detail: {
      empty: 'No data for this period.',
      occurrences: 'occurrences',
      validationRate: 'Validation rate',
      statusAll: 'All',
    },
    emotionsTab: {
      mostFrequent: 'Most frequent',
      byIntensity: 'By intensity',
      subtle: 'Subtle',
      moderate: 'Moderate',
      strong: 'High',
    },
    beliefsTab: {
      recurrent: 'Recurring beliefs',
    },
    patternsTab: {
      detected: 'Detected patterns',
      severity: 'Severity',
      status: 'Status',
    },
    menu: {
      viewAsUser: 'View as user',
      signOut: 'Sign out',
    },
    users: {
      placeholderTitle: 'User management',
      placeholderBody:
        'Coming soon: a list of users, subscription status, signup dates and moderation actions.',
      searchPlaceholder: 'Search by name or email...',
      count_one: '{{count}} user',
      count_other: '{{count}} users',
      emptyAll: 'No users registered yet.',
      emptySearch: 'No users found for that search.',
      entries: 'reg',
    },
    sections: {
      users: 'Users',
      engagement: 'Engagement',
      emotions: 'Emotions',
      thoughts: 'Thoughts',
      beliefs: 'Beliefs',
      patterns: 'Patterns',
      feedback: 'User suggestions',
    },
    metrics: {
      totalUsers: 'Total users',
      usersTrial: 'In trial period',
      usersActive: 'Active subscribers',
      subsAnnual: 'Annual subscriptions',
      subsMonthly: 'Monthly subscriptions',
      usersApple: 'Via Apple',
      usersGoogle: 'Via Google Play',
      totalEntries: 'Total entries',
      avgWeekly: 'Weekly average per user',
      suggested: 'Suggested',
      pending: 'Pending',
      validated: 'Validated',
      rejected: 'Rejected',
      edited: 'Edited',
      ignored: 'Ignored',
    },
    hints: {
      requiresSubscriptions: 'Waiting for subscription module',
      requiresStoreData: 'Waiting for store integration',
      requiresThoughtsValidation: 'Waiting for thoughts validation schema',
    },
    feedback: {
      filterAll: 'All',
      filterNew: 'New',
      filterRead: 'Read',
      filterResolved: 'Resolved',
      empty: 'No suggestions received yet.',
      noName: 'Unnamed user',
      statusNew: 'New',
      statusRead: 'Read',
      statusResolved: 'Resolved',
      markAs: 'Mark as',
    },
  },
  nav: {
    home: 'Home',
    history: 'History',
    insights: 'Insights',
    summary: 'Summary',
    profile: 'Profile',
  },
  entitlement: {
    limitReached:
      'You reached the limit of {{limit}} entries for this period. Upgrade your plan to continue.',
    paywallTitle: 'Access restricted',
    paywallBodyTrialDone:
      'Your trial has ended. Subscribe to a plan to keep creating entries and summaries.',
    paywallBodyGeneric:
      'Your subscription is not active. Subscribe to a plan to create entries and summaries.',
    paywallSeeInsights: 'See my insights',
    backHome: 'Back to home',
  },
  plans: {
    salesTitle: 'Continue your journey',
    salesSubtitle:
      'Pick a plan and keep journaling, spotting patterns and caring for yourself with Caminare.',
    trialDoneTitle: 'Your trial has ended',
    trialDoneSubtitle:
      'Subscribe to Caminare to keep using every feature for just {{monthly}}/month, or the annual plan with {{pct}}% off for {{annual}}/year.',
    restrictedTitle: 'Subscribe to continue',
    restrictedSubtitle:
      'Your subscription is not active. Choose a plan to start creating entries and summaries again.',
    cadenceMonthly: 'Monthly',
    cadenceAnnual: 'Annual',
    annualBadge: '−{{pct}}%',
    promoBadge: 'First-time offer · −{{pct}}%',
    tierBasico: 'Basic',
    tierAvancado: 'Advanced',
    basicoTagline: 'To keep the habit of checking in with yourself.',
    avancadoTagline: 'For those who journal every day.',
    limit: 'up to {{count}} entries per month',
    perMonthPrice: '{{price}}',
    perMonthSuffix: '/mo',
    perYearPrice: '{{price}}',
    perYearSuffix: '/yr',
    annualEquiv: 'that’s {{price}}/month',
    annualSaving: '{{pct}}% off on the annual plan',
    promoLine: 'First-time offer: {{price}}/year ({{perMonth}}/month)',
    recommended: 'Recommended',
    bestValue: 'Best value',
    subscribe: 'Subscribe',
    subscribing: 'Processing…',
    restore: 'Restore purchases',
    restoring: 'Restoring…',
    // Auto-renewable subscription requirements (Apple/Google)
    autoRenew:
      '{{cadence}} subscription with automatic renewal. Billing goes through your store account and renews at the end of each period unless cancelled at least 24h before it ends.',
    cadenceMonthlyWord: 'Monthly',
    cadenceAnnualWord: 'Annual',
    manageApple: 'You can manage or cancel in Settings › Apple ID › Subscriptions.',
    manageGoogle: 'You can manage or cancel in the Play Store › Subscriptions.',
    legalIntro: 'By subscribing, you agree to our',
    legalAnd: 'and',
    purchaseError: 'We couldn’t complete the purchase. Please try again.',
    restoreSuccess: 'All set! Your purchases have been restored.',
    restoreNone: 'We couldn’t find previous purchases to restore.',
    restoreError: 'We couldn’t restore purchases right now.',
    // Store showcase loading (native app)
    loadingStore: 'Loading plans…',
    loadError: 'We couldn’t load the plans. Check your connection and try again.',
    retry: 'Try again',
  },
  trialNotices: {
    day7Title: 'Enjoying Caminare?',
    day7Body:
      'You have {{days}} more free days — subscribe now for just {{monthly}}/month, or the annual plan with {{pct}}% off for {{annual}}/year.',
    day14Title: 'Your trial ends tomorrow',
    day14Body:
      'Subscribe to Caminare to keep using every feature for just {{monthly}}/month, or the annual plan with {{pct}}% off for {{annual}}/year.',
    cta: 'See plans',
    dismiss: 'Not now',
  },
} as const;
