...
export async function deleteAccount(): Promise<{ error: string | null }> {
  try {
    // A função delete_my_account (SECURITY DEFINER) apaga os dados E o próprio
    // usuário em auth.users. Não precisa de passo extra pra remover o auth.
    const { error } = await supabase.rpc('delete_my_account');
    if (error) {
      console.error('[db.deleteAccount] rpc', error);
      return { error: error.message };
    }
    // O usuário de auth já não existe; encerra a sessão local. Revogar no servidor
    // pode falhar (usuário sumiu) — por isso o catch, mas o storage local é limpo.
